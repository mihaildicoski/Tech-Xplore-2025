...
export class MyMCP extends McpAgent {
	...
	async init() {
		...
		this.server.tool(
			...
			async () => {
			  const response = await fetch("http://localhost:8787/api/financial-advice"); 
			  if (!response.ok) {
...
this.server.tool(
  ...
  this.server.tool(
	...
	async ({ count }) => {
	  ...
	  const transactionsToUse = count ? data.transactions.slice(0, count) : data.transactions;
  
	  const transactionList = transactionsToUse.map(tx =>
	  ...
	  const totalSpent = transactionsToUse.reduce((sum, tx) => sum + tx.amount, 0);
	  const totalCarbon = transactionsToUse.reduce((sum, tx) => sum + tx.carbonImpact, 0);
	  const recurringCount = transactionsToUse.filter(tx => tx.isRecurring).length;
...
this.server.tool(
	...
	async () => {
	  ...
	  const summary = data.investments.map(inv => `
  ...
  this.server.tool(
	"getCarbon", 
	"Retrieves the carbon footprint in carbonKg, the monthly breakdown which gives amount per month, and then a breakdown which lists the use per category. F"
  )
